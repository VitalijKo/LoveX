...
class UserInfo:
    ...
    def UserInfo(self):
        ...
        self.verification = self.json.get('vf')
...
class Warns:
    def __init__ (self, databalance):
        self.json = data
        self.left_time_of_ban=None
        self.left_time_when_can_communicate=None

    @property
    def Warns(self):
        self.left_time_of_ban = self.json.get('leftTimeOfBan')
        self.left_time_when_can_communicate = self.json.get('leftTimeWhenCanCommunicate')
...
class Users:
    def __init__(self, data):
        ...
        self.first_name = None
        self.photo_100 = None
        self.harem_total_price = None
        self.verification = None
        self.hp = None
        self.invisible_timestamp = None
    ...
    def Users(self):
        ...
        self.photo_100 = self.json.get('p100')
        self.verification = self.json.get('vf')
        self.harem_total_price = self.json.get('haremTotalPrice')
        self.hp = self.json.get('hp')
        self.invisible_timestamp = self.json.get('invisTs')
...
class Top:
    ...
    def Top(self):
        ...
        for user in self.json.get('users3', []):
            self.users.append(Users(user).Users)
...
class Search:
    ...
    @property
    def Search(self):
        for user in self.json.get('part0', []):
            self.part_0.append(SearchUser(user).SearchUser)

        for user in self.json.get('part1', []):
            self.part_1.append(SearchUser(user).SearchUser)

        for user in self.json.get('part2', []):
